...
def get_evolution_config(input_configs):
    global configs
    configs.clear()
    for k in input_configs:
        configs[k] = input_configs[k]
...
def load_config_from_file(file_path: str) -> None:
    ...
    for i_level in range(len(levels)):
        ...
        if os.path.exists(cur_config_path):
            with open(cur_config_path, 'r') as f:  # yaml专用于写配置文件
                _config = yaml.safe_load(f)
                _iterative_update(configs, _config)  # 将yaml文件中读取的内容（已存在_config中）放入config内
