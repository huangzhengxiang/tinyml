...
from core.utils.config import configs, get_evolution_config, load_config_from_file, update_config_from_args, update_config_from_unknown_args
...
def build_config():  # separate this config requirement so that we can call main() in ray tune
    ...
    update_config_from_args(args)  # 解析parse中已经定义的参数（如rundir）
    update_config_from_unknown_args(unknown)  # 解析parse中未定义的参数（如net_name）
'''


def build_config(input_configs):
    get_evolution_config(input_configs)
